...
most.popular.name.2013 = name[prop == max(prop.2013)]
...
MostPopular <- function(my.year) {
  paste("The most popular name in", my.year, "was", name[prop == max(prop[year == my.year])])
}
...
HowPopular <- function(my.name, my.year) {
  rel.prop <- name[prop == prop[year == my.year]]
}
